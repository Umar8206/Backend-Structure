...
import { findUserByEmailOrPhone, updateUserByEmail } from '../repositries/user.js'
...
export const sendOtpService = async (payload) => {
  ...
  if (payload?.email && payload.method == otpMethods.email) {
    const filePath = path.join(__dirname, '../templates/', createPayload?.type==otpTypes.registration?`otp.html`:`forgotPassword.html`)
